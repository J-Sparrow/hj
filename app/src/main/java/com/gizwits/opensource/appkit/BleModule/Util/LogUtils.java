...
/**
 * Created by connli on 2017/12/7.
 */
...
public class LogUtils {
     ...
     * 存放日志文件的所在路径
     */
     private static final String DIRPATH = "/ringDebugLog";
    /**
     * 存放日志的文本名
     */
     public static final String LOGNAME = "ringlog";
    /**
     * 设置时间的格式
     */
     ...
     * @return void 返回类型
     */
    public static void ow(String tag, String msg) {
//        if (isDeBug) {
//            Log.w(tag, msg);
//        }
        if (isWrite) {
    ...
    public static void write(String tag, String msg) {
        String path = FileUtils.createMkdirsAndFiles(DIRPATH, getLogname());
    ...
    private static String getLogname(){
        ...
        name = LOGNAME+str+".log";
        return name;
    }
    private static ArrayList<String> getLogNames(){
        ArrayList<String> names = new ArrayList<>();
        for(int i=0;i<7;i++){
            String name = "";
            SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMdd");
            Date curDate = new Date(System.currentTimeMillis()-86400000*i);//获取当前时间
            String str = formatter.format(curDate);
            name = LOGNAME+str+".log";
            names.add(name);
        }
    ...
    public static void cleanLog() {
        try {
            String str = "";
            File path = new File(FileUtils.getSDCardRoot()+DIRPATH);
            File[] files = path.listFiles();// 读取文件夹下文件
            ArrayList<String> logNames=getLogNames();
            if (files != null) { // 先判断目录是否为空，否则会报空指针
                for (File file : files) {
                    if (file.isDirectory()){//检查此路径名的文件是否是一个目录(文件夹)
//                    LogUtils.i("zeng", "若是文件目录。继续读1"
//           g                 +file.getName().toString()+file.getPath().toString());
//                    getFileName(file.listFiles());
//                    LogUtils.i("zeng", "若是文件目录。继续读2"
//                            +file.getName().toString()+ file.getPath().toString());
                    } else {
                        boolean neetDelete = true;

...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"airHumi"对应的标识名
	protected static final String KEY_AIRHUMI = "airHumi";
	// 数据点"soilHumi"对应的标识名
	protected static final String KEY_SOILHUMI = "soilHumi";
	// 数据点"illumination"对应的标识名
	protected static final String KEY_ILLUMINATION = "illumination";
	// 数据点"airTemp"对应的标识名
	protected static final String KEY_AIRTEMP = "airTemp";
	// 数据点"soilTemp"对应的标识名
	protected static final String KEY_SOILTEMP = "soilTemp";
	// 数据点"co2Conc"对应的标识名
	protected static final String KEY_CO2CONC = "co2Conc";
	 ...
	 */
	// 数据点"airHumi"对应的存储数据
	protected static int data_airHumi;
	// 数据点"soilHumi"对应的存储数据
	protected static int data_soilHumi;
	// 数据点"illumination"对应的存储数据
	protected static int data_illumination;
	// 数据点"airTemp"对应的存储数据
	protected static double data_airTemp;
	// 数据点"soilTemp"对应的存储数据
	protected static double data_soilTemp;
	// 数据点"co2Conc"对应的存储数据
	protected static int data_co2Conc;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_AIRHUMI)) {
			
					data_airHumi = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_SOILHUMI)) {
			
					data_soilHumi = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_ILLUMINATION)) {
			
					data_illumination = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_AIRTEMP)) {
			
					if (map.get(dataKey) instanceof Integer) {
						data_airTemp = (Integer) map.get(dataKey);
					} else {
						data_airTemp = (Double) map.get(dataKey);
					}
				}
				if (dataKey.equals(KEY_SOILTEMP)) {
			
					if (map.get(dataKey) instanceof Integer) {
						data_soilTemp = (Integer) map.get(dataKey);
					} else {
						data_soilTemp = (Double) map.get(dataKey);
					}
				}
				if (dataKey.equals(KEY_CO2CONC)) {
			
					data_co2Conc = (Integer) map.get(dataKey);
				}
			}
		}
